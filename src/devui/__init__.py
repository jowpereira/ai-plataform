...
"""MAIA - Debug interface with OpenAI compatible API server."""
...
def serve(
    ...
    if host not in ("127.0.0.1", "localhost") and not auth_enabled:
        logger.warning("⚠️  WARNING: Exposing MAIA to network without authentication!")
    ...
    logger.info(f"Starting MAIA on {host}:{port}")
